...
export default function ProblemsPointsStep({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold text-slate-800 dark:text-white mb-1">Point System</h3>
        <p className="text-sm text-slate-600 dark:text-slate-400">Choose how points will be assigned to problems</p>
      </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="easyPoints" className="text-slate-700 dark:text-slate-300 font-medium flex items-center gap-2">
                <span className="px-2 py-0.5 text-xs font-semibold rounded bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 border border-green-200 dark:border-green-800">
                  Easy
                </span>
            ...
            {/* Average Points */}
            <div className="space-y-2">
              <Label htmlFor="averagePoints" className="text-slate-700 dark:text-slate-300 font-medium flex items-center gap-2">
                <span className="px-2 py-0.5 text-xs font-semibold rounded bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 border border-yellow-200 dark:border-yellow-800">
                  Average
                </span>
                Points <span className="text-red-500">*</span>
              </Label>
            ...
            {/* Difficult Points */}
            <div className="space-y-2">
              <Label htmlFor="difficultPoints" className="text-slate-700 dark:text-slate-300 font-medium flex items-center gap-2">
      ...
      <div className="space-y-4 pt-6 border-t border-slate-200 dark:border-slate-700">
        <div>
          <h3 className="text-lg font-semibold text-slate-800 dark:text-white mb-1">Problem Selection</h3>
          <p className="text-sm text-slate-600 dark:text-slate-400">Choose problems from your problem banks</p>
        </div>
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <h4 className="font-medium text-slate-800 dark:text-white">{bank.title}</h4>
                      {bank.description && (
                        <p className="text-sm text-slate-600 dark:text-slate-400 mt-1 line-clamp-1">{bank.description}</p>
                      )}
                    </div>
        ...
        {selectedBankId && (
          <div className="space-y-2">
            ...
            {isLoadingProblems ? (
              ...
              <div className="text-center py-8 text-slate-500 dark:text-slate-400 border-2 border-dashed border-slate-200 dark:border-slate-600 rounded-lg">
              ...
              <div className="border border-slate-200 dark:border-slate-700 rounded-lg divide-y divide-slate-200 dark:divide-slate-700 max-h-96 overflow-y-auto">
                {bankProblems.map((problem) => {
                  ...
                  return (
                    <div
                      key={problem.id}
                      className={`p-4 ${isSelected ? "bg-[#f49700]/5" : "bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700"} transition-colors`}
                    >
                      <div className="flex items-start gap-3">
                        <input
                          type="checkbox"
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-slate-800 dark:text-white mb-2 line-clamp-2"><MathRenderer text={problem.question} /></p>
          ...
          <div className="space-y-2">
            ...
            <div className="border border-slate-200 dark:border-slate-700 rounded-lg divide-y divide-slate-200 dark:divide-slate-700">
              {selectedProblems.map((sp, index) => (
                <div key={sp.problem.id} className="p-4 bg-white dark:bg-slate-800">
                  <div className="flex items-start gap-3">
                    <div className="flex-shrink-0 w-8 h-8 bg-slate-100 dark:bg-slate-700 rounded-full flex items-center justify-center text-sm font-medium text-slate-600 dark:text-slate-300">
                      {index + 1}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="text-sm text-slate-800 dark:text-white mb-2 line-clamp-2"><MathRenderer text={sp.problem.question} /></p>
                      ...
                      {formData.pointSystemType === "manual" && (
                        <div className="flex items-center gap-2">
                          <Label htmlFor={`points-${sp.problem.id}`} className="text-xs text-slate-600 dark:text-slate-400">
                            Points:
                          </Label>
                          <Input
