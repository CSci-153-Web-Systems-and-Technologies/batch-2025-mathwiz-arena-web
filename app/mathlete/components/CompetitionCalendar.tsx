    ...
    calendarDays.push(
      <div
        ...
        className={`h-7 sm:h-8 flex items-center justify-center text-xs sm:text-sm rounded-lg relative group cursor-pointer
          ...
          ${!hasCompetition && !isToday ? "text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700" : ""}
        `}
        title={hasCompetition ? competitions.map(c => c.name).join(", ") : ""}
      >
        {day}
        {hasCompetition && (
          <div className={`absolute bottom-0.5 left-1/2 transform -translate-x-1/2 w-1 h-1 rounded-full ${isToday ? "bg-purple-600" : "bg-red-600"
            }`}></div>
        )}
      </div>
    );
