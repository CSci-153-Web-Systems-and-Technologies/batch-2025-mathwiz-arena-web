...
interface MathleteSidebarProps {
  notificationCount?: number;
}

export default function MathleteSidebar({ notificationCount = 0 }: MathleteSidebarProps) {
  ...
  const SidebarContent = ({ showCloseButton = false }: { showCloseButton?: boolean }) => (
    <div className="p-4 sm:p-6">
      ...
      <Link href="/" className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-base sm:text-lg font-semibold text-[#25346A] dark:text-white">Mathwiz</h1>
      ...
      <nav className="space-y-1">
        {navItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className={`flex items-center gap-3 px-3 sm:px-4 py-2.5 sm:py-3 text-sm font-medium rounded-lg transition-colors ${isActive(item.href)
    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 py-3 flex items-center gap-3">
        <button
          ...
          className="mobile-menu-button p-2 -ml-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
      ...
      {/* Mobile Sidebar */}
      <aside
        className={`sidebar-container lg:hidden fixed top-0 left-0 h-full w-56 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 z-50 transform transition-transform duration-300 ease-in-out overflow-y-auto ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
      ...
      {/* Desktop Sidebar */}
      <aside className="sidebar-container hidden lg:block w-64 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 fixed h-full overflow-y-auto transition-colors">
        <SidebarContent />
      </aside>
    </>
  );
