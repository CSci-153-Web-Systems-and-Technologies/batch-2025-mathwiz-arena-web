...
export default function ProfileHeader({ profile, stats, isOwnProfile, onEditProfile, activeTab: externalActiveTab, onTabChange }: ProfileHeaderProps) {
    ...
    const ActionButtons = ({ isMobile = false }: { isMobile?: boolean }) => (
        <>
            ...
            {/* More options button */}
            <div className="relative" ref={isMobile ? undefined : moreMenuRef}>
                <button
                    onClick={() => setShowMoreMenu(!showMoreMenu)}
                    className="w-10 h-10 bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors flex items-center justify-center flex-shrink-0"
                >
                ...
                {/* Dropdown Menu */}
                {showMoreMenu && (
                    <div className={`absolute top-12 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 py-2 z-50 ${isMobile ? 'left-0 right-0 w-full' : 'right-0 w-56'}`}>
                        <Link
