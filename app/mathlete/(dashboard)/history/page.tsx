...
export default async function MathleteHistoryPage() {
  ...
  const formatDuration = (startedAt: string, endedAt: string | null, durationMinutes: number) => {
    if (!endedAt) {
      return `${durationMinutes} min (limit)`;
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
      <div className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <h1 className="text-3xl font-bold text-[#25346A] dark:text-white">Competition History</h1>
          <p className="text-slate-600 dark:text-slate-400 mt-1">View your past competitions and performance</p>
        </div>
      </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-800 dark:text-white">{completedAttempts.length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-800 dark:text-white">{totalScore}</p>
              </div>
            </div>
          </div>
        ...
        {attempts && attempts.length > 0 ? (
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700">
              <h2 className="text-lg font-semibold text-slate-800 dark:text-white">All Attempts</h2>
            </div>
            <div className="divide-y divide-slate-200 dark:divide-slate-700">
              {attempts.map((attempt) => {
                const competition = attempt.competitions as any;
                return (
                  <div key={attempt.id} className="px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        {attempt.is_completed ? (
                          <Link
                            ...
                            className="px-4 py-2 text-sm font-medium text-[#25346A] bg-[#25346A]/10 rounded-lg hover:bg-[#25346A]/20 transition-colors"
                          >
                            View Results
                          </Link>
                        ) : (
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <div className="p-12 text-center">
              <div className="w-20 h-20 bg-[#25346A]/10 dark:bg-blue-900/30 rounded-full flex items-center justify-center mx-auto mb-6">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 text-[#25346A] dark:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
              </div>
