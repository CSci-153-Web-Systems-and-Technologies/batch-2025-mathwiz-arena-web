...
export default function MathInput({
    ...
    return (
        <div className={`space-y-2 ${className}`}>
            {/* Toolbar */}
            <div className="flex flex-wrap items-center gap-1 p-2 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-t-md">
                    ...
                    <div key={category} className="relative">
                        <button
                            ...
                            className={`px-2 py-1 text-xs font-medium rounded transition-colors ${activeCategory === category
                                ...
                                : "bg-white dark:bg-slate-600 text-slate-600 dark:text-slate-200 border border-slate-200 dark:border-slate-500 hover:border-slate-300 dark:hover:border-slate-400"
                                } disabled:opacity-50`}
                        >
                            {category}
                        </button>
                        ...
                        {activeCategory === category && (
                            <div className="absolute top-full left-0 mt-1 z-10 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-md shadow-lg p-2 flex flex-wrap gap-1 min-w-[200px]">
                                {symbols.map((symbol, idx) => (
                                    <button
                                        ...
                                        title={symbol.title}
                                        className="w-8 h-8 flex items-center justify-center text-sm font-medium bg-slate-50 dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600 border border-slate-200 dark:border-slate-600 rounded transition-colors text-slate-700 dark:text-slate-200"
                                    >
                                        {symbol.label}
                                    </button>
                                ))}
            ...
            <textarea
                ...
                disabled={disabled}
                required={required}
                className={`w-full px-3 py-2 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white rounded-b-md focus:outline-none focus:ring-2 focus:ring-[#f49700] focus:border-transparent resize-none placeholder:text-slate-400 dark:placeholder:text-slate-500`}
                style={{ minHeight }}
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
                💡 Tip: Wrap math expressions in dollar signs. Example: <code className="bg-slate-100 dark:bg-slate-700 px-1 rounded text-slate-700 dark:text-slate-300">$x^2 + y^2 = z^2$</code>
            </p>
            ...
            {showPreview && value && (
                <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md">
                    ...
                    <div
                        className="text-slate-800 dark:text-slate-200 prose prose-sm max-w-none"
        ...
        <div className={`flex-1 ${className}`}>
            <div className="flex items-center gap-2">
                <input
                    ...
                    disabled={disabled}
                    required={required}
                    className="flex-1 h-10 px-3 py-2 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white rounded-md focus:outline-none focus:ring-2 focus:ring-[#f49700] focus:border-transparent text-sm placeholder:text-slate-400 dark:placeholder:text-slate-500"
                />
            ...
            {showPreview && value && (
                <div className="mt-1 p-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded text-sm">
                    <MathRenderer text={value} className="text-slate-800 dark:text-slate-200" />
                </div>
            )}
        </div>
    );
